...
calculate_firstder (flowpath * current_flowpath)
{
  ...
  if ((firstder =
       (double *)calloc (current_flowpath->size (), sizeof (double))) == NULL)
...
double *
calculate_curvature (flowpath * current_flowpath, double *firstder, double *y2)
{
  double *curvature;
...
calculate_asymmetry (flowpath * current_flowpath, double *curvature)
{
       ...
       i != current_flowpath->end (); i++, j++)
  {
    ...
    asymmetry[j] =
      ((curvature[j] - max_curvature) + (curvature[j] - min_curvature)) / range;
