...
flowpath::iterator
determine_coastline_cell(flowpath* current_flowpath, double sealevel, long sim_time)
{
	flowpath::iterator i;
...
double*
determine_discharge_along_flowpath(flowpath* current_flowpath, flowpath::iterator coastline_cell_index, double sealevel, long sim_time)
{
	//DISCHARGE DETERMINATION
	double* discharge;
	if ((discharge=(double*)calloc(current_flowpath->size () + 1,sizeof(double)))==NULL)
	...
	{	
		...
		discharge[j] = discharge[j-1] - (discharge_shapef * dx * waterdepth * discharge[j-1]);
...
double*
determine_erosion_rate_along_flowpath(flowpath* current_flowpath, flowpath::iterator coastline_cell_index, double *discharge, long sim_time)
{
	...
	double* dh_erosion;
	if ((dh_erosion = (double*)calloc (current_flowpath->size () + 1,sizeof(double)))==NULL)
	...
	if (nextCell != current_flowpath->end()) nextCell++;

	for (flowpath::iterator i = current_flowpath->begin(); i != current_flowpath->end (); i++, j++,nextCell++)

	{
		...
		if ( nextCell != current_flowpath->end ()) 
			{
                slope = (get_topo_height(sim_time, i->row, i->col) - get_topo_height (sim_time, nextCell->row,nextCell->col)) / dx;
            } 
...
double**
dig_erosion_from_space(flowpath* current_flowpath, double *dh_erosion, long sim_time)
{
	double** sedflux_ero;
	if ((sedflux_ero=(double**)calloc( current_flowpath->size ()+1, sizeof(double*)))==NULL)
		printf("error in initializing erosional sedflux array");

	for (int k=0;k < current_flowpath->size ()+1;k++)
	{
		if ((sedflux_ero[k]=(double*)calloc(number_of_gscl,sizeof(double)))==NULL)
			printf("error in initializing erosional sedflux array");
	}
		...
		for (long m = sim_time  - 1; m >= 0 && !done; m--)
		{
			...
			if (height != 0) {
				...
				else
				{
					...
					for (int l = 1; l <= number_of_gscl; l++)
					{
						double grain_percentage = grain_percent (m, i->row, i->col, l) * dh_erosion[j];
...
double** sedimentation_rate_along_flowpath(flowpath* current_flowpath,flowpath::iterator coastline_cell_index, double** sedflux_ero)
{
	...
	double** sedflux_depo;
	if ((sedflux_depo=(double**)calloc( current_flowpath->size ()+1, sizeof(double*)))==NULL)
		printf("error in initializing depositional sedflux array");

	for (int k=0;k < current_flowpath->size ()+1;k++)
	{
		if ((sedflux_depo[k]=(double*)calloc(number_of_gscl,sizeof(double)))==NULL)
			printf("error in initializing depositional sedflux array");
	}
...
// SEDIMENTATION DETERMINATION (GRAINSIZE SPECIFIC)
	...
	{	
		for (n=0;n<number_of_gscl; n++)
		{
			...
			settle_rate = (sedflux_depo[j-1][n]+sedflux_ero[j-1][n])/traveldist;
