...
void open_data_files(FILE **ftopo, FILE **fpath, int time)
{
	char filename[100];
...
{	
	...
	if ((median=(double **) calloc((end_of_times/dt),sizeof(double *))) == NULL) 	printf("median array could not be allocated");
	for(i=0;i<(end_of_times/dt);i++)
	{
		if ((median[i]=(double *) calloc(nx,sizeof(double))) == NULL) printf("median array could not be allocated")	;
	}
...
{	
	...
	// allocation of median
	if ((median_point=(double *) calloc((end_of_times/dt),sizeof(double))) == NULL) 	printf("median array could not be allocated");
			...
			if(space[j][wellpos_row][wellpos_col][0]!=0)
				{
					...
					for(k=1;k<=number_of_gscl;k++)
					{
						tmpsum += (space[j][wellpos_row][wellpos_col][k-1] * grain_size[k-1]);
						median_point[j] = 10000*(tmpsum);///space[j][wellpos_row][wellpos_col][0]);  // in micron
						cout<<"time "<<j<<" "<<tmpsum<<" depth "<<space[j][wellpos_row][wellpos_col][0] <<" median "<<median_point[j]<<endl;			
					}
				
				}
				else median_point[j] = 4;
...
void write_median_psfile(double **median, int Xpos)
{
	...
	long hhmin,hhmax;
	double hmin,hmax,scx,scy,huemedian,medianmin,medianmax,medianminn,medianmaxx,dmedian;
	FILE *psfp;
	char f_name[255];
	...
	fprintf(psfp,"100 -8 moveto 0 0 0 setrgbcolor (Median Grainsize [mu]) show\n");	
	fprintf(psfp,"grestore\n");
	...
	for (i=0; i<(end_of_times/dt); i++) 
	{
		for(j=xstrtps;j<xstopps;j++)
		{
			...
			if (i>0)
			{
				...
				//hier aangepast omdat topografische hoogte gebruikt moet worden?
				fprintf(psfp,"%5.3f 1.0 1.0 sethsbcolor\n",huemedian); //COLOUR
				fprintf(psfp,"%5.1f %5.1f moveto\n",Lmarg+scx*(j-xstrtps),  Bmarg+scy*(get_topo_height(i,Xpos,j)-hmin));
				fprintf(psfp,"%5.1f %5.1f lineto\n",Lmarg+scx*(j+1-xstrtps),Bmarg+scy*(get_topo_height(i,Xpos,j+1)-hmin));
				fprintf(psfp,"%5.1f %5.1f lineto\n",Lmarg+scx*(j+1-xstrtps),Bmarg+scy*(get_topo_height(i-1,Xpos,j+1)-hmin));
				fprintf(psfp,"%5.1f %5.1f lineto\n",Lmarg+scx*(j-xstrtps),  Bmarg+scy*(get_topo_height(i-1,Xpos,j)-hmin));
				fprintf(psfp,"%5.1f %5.1f lineto\n",Lmarg+scx*(j-xstrtps),  Bmarg+scy*(get_topo_height(i,Xpos,j)-hmin));	
				fprintf(psfp,"fill\n");
			...
			if (i==0)	/* put tickmarks every j and labels every 5 j	*/
			{
				...
				if (j%1==0){								/*outline tickmarks according to number*/
...
void write_simulatedwell_psfile(double *median_point, int wellpos_row,int wellpos_col)
{
	...
	// Draw well FILL 
		...
		fprintf(psfp,"%5.1f %5.1f moveto\n",Lmarg+scx,Bmarg+scy*(get_topo_height(end_of_times/dt-1, wellpos_row, wellpos_col)-hmin));
		fprintf(psfp,"%5.1f %5.1f lineto\n",Lmarg+scx,Bmarg+scy*(get_topo_height(0, wellpos_row, wellpos_col)-hmin));
		...
		for (i=1;i<end_of_times/dt;i++)
		{
			...
			{		
				...
				fprintf(psfp,"%5.1f %5.1f lineto\n",(0.30*(median_point[i]))+Lmarg+scx, Bmarg+scy*(get_topo_height(i-1, wellpos_row, wellpos_col)-hmin));
				fprintf(psfp,"%5.1f %5.1f lineto\n",(0.30*(median_point[i]))+Lmarg+scx, Bmarg+scy*(get_topo_height(i, wellpos_row, wellpos_col)-hmin));
				fprintf(psfp,"%5.1f %5.1f lineto\n",Lmarg+scx , Bmarg+scy*(get_topo_height(i, wellpos_row, wellpos_col)-hmin));
				fprintf(psfp,"%5.1f %5.1f lineto\n",Lmarg+scx, Bmarg+scy*(get_topo_height(i-1, wellpos_row, wellpos_col)-hmin));
